...
# --------------------------------------------
# Paths
var settingspath = OS.get_user_data_dir() + "/settings.uci"
var projectpath = OS.get_user_data_dir() + "/project.uci"
#var pluginspath = OS.get_user_data_dir() + "/plugins/"
# --------------------------------------------
...
# Run on load
func _ready():
	if file.file_exists("res://settings.uci"):
		if not file.file_exists(settingspath):
			dir.copy("res://settings.uci", settingspath)
			dir.remove("res://settings.uci")
...
func fileSave(isDirect:bool):
	...
	if isDirect == true:
		...
		file.open(OS.get_user_data_dir() + "/.dot", File.WRITE)
